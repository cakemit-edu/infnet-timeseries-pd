...
(ts1 <- readr::read_delim("_datasets/tabela1620.csv", delim=",", show_col_types=F, skip=4) |> 
  ...
  pivot_longer(2:23, names_to="name", values_to="producao") |> # Coloca as séries empilhadas
  ...
  # Finaliza a tabela
  select(Trimestre, name, producao) |> 
...
```{r}
ts1 |> features(producao, features = feature_set(tags = "unitroot"))
```
...
Nesse caso foi inicialmente aplicada uma decomposição aditiva nos dados. Na figura a seguir os três componentes - sazonalidade, tendência e resíduo - são mostrados separadamente nos três painéis inferiores. Na decomposição aditiva esses componentes podem ser somados para reconstruir os dados originais mostrados no primeiro painel. 

As barras cinzas à esquerda de cada painel mostram as escalas relativas dos componentes. Cada barra cinza representa o mesmo comprimento, mas como os gráficos estão em escalas diferentes, as barras variam em tamanho. A grande barra cinza no painel de Tendência mostra que a variação neste componente é menor em comparação com a variação nos dados originais do primeiro painel. Se encolhermos os três painéis inferiores até que suas barras fiquem do mesmo tamanho que as do painel dos dados originais, todos os painéis estariam na mesma escala.
...
Chama atenção também a similaridade visual entre o comportamento da sazonalidade e da tendência do setor industrial e de serviços e sua distinção em relação ao setor agropecuário. 


```{r fig.asp=1}
ts1 |> 
  model(STL = STL(producao)) |> 
  components() |> 
  autoplot() +
  theme(legend.position="top") +
  scale_x_yearquarter(date_breaks="5 years") +
  scale_color_brewer(palette="Dark2") +
  labs(title="Decomposição aditiva da série de produção por trimestre")
```


O componente sazonal também é mais forte no setor agropecuário com maior amplitude e com periodicidade bem marcada. Faz sentido, já que o setor agropecuário pode ser mais sensível a fatores sazonais, como a época de colheita ou condições climáticas.

A amplitude da sazonalidade (a diferença entre picos e vales) no setor agropecuário tende a aumentar progressivamente. Isso também ocorre, porém em menor amplitude, nos setores industrial e de serviços. A seguir aplicamos uma decomposição multiplicativa, que é mais adequada quando a amplitude da sazonalidade aumenta ou diminui em linha com aumento ou diminuição no nível da série. 

A decomposição multiplicativa deixa a sazonalidade mais estável e faz com que as componentes de tendência e sazonalidade expliquem uma parcela maior da variabilidade dos dados. Dessa forma, o resíduo fica com picos e vales menores (uma variância mais estável) do que na decomposição aditiva - especialmente comparado à escala do resíduo na decomposição aditiva. 

A tendência continua a mostrar um crescimento quase linear no setor agropecuário e superior aos outros setores, mas parece surgir a ocorrência de dois vales em 2016 e 2022. Talvez seja o início de um comportamento cíclico com defasagem de 6 anos. Mudanças climáticas?

Nos setores industrial e de serviços a tendência de crescimento sofre estagnação a partir de 2014, mas o segmento de serviços parece mostrar indícios de recuperação, enquanto a indústria segue estagnada.
...
-   Critério de Informação de Akaike ou *Akaike Information Criterion* (AIC) de $671,81$. Esse coeficiente penaliza a medida de ajuste do modelo com a quantidade de parâmetros que precisam ser estimados. Quanto mais parâmetros, maior o AIC. Quanto menor o AIC, melhor o modelo. 

-   AICC é o AIC corrigido para o tamanho da amostra, com valor de $672,27$. Semelhante ao AIC, ajusta o critério para o número de observações e é especialmente útil quando há poucas observações.

-   Critério de Informação Bayesiano ou *Bayesian Information Criterion* (BIC), com valor de $681,9$, penaliza pela quantidade de parâmetros utilizados, favorecendo modelos mais simples. O modelo escolhido pelo BIC é geralmente o mesmo escolhido pelo AIC ou um com menos termos. Isso ocorre porque o BIC penaliza mais fortemente o número de parâmetros do que o AIC.
...
fit.agr <- ts1.train |> 
  model(
    autoARIMA = ARIMA(producao ~ pdq(0,0,1) + PDQ(0,1,1,4)), # Modelo autoarima
    ARIMA_010.010 = ARIMA(producao ~ pdq(0,1,0) + PDQ(0,1,0,4)), # Random Walk (naive forecast)
    ARIMA_000.010 = ARIMA(producao ~ pdq(0,0,0) + PDQ(0,1,0,4)), # só diferenciação sazonal
    ARIMA_000.011 = ARIMA(producao ~ pdq(0,0,0) + PDQ(0,1,1,4)), # diferenciação com MA sazonal
    ARIMA_000.110 = ARIMA(producao ~ pdq(0,0,0) + PDQ(1,1,0,4)), # diferenciação com AR sazonal
    ARIMA_000.111 = ARIMA(producao ~ pdq(0,0,0) + PDQ(1,1,1,4)), # diferenciação com AR e MA sazonal
    ARIMA_101.011 = ARIMA(producao ~ pdq(1,0,1) + PDQ(0,1,1,4)), # Modelo autoarima + AR1
  )

fit.agr |> pivot_longer(!name, names_to="Modelo", values_to = "Parâmetros")
...
a. Os resíduos de inovação têm variância constante. Isso é conhecido como “homoscedasticidade”.

b. Os resíduos de inovação são normalmente distribuídos.
...
```{r}
gg_tsresiduals(fit.agr |> select(autoARIMA))
```
...
Grandes valores de Q∗ sugerem que as autocorrelações não provêm de uma série de ruído branco. Quão grande é muito grande? Se as autocorrelações viessem de uma série de ruído branco, então Q e Q∗ teriam uma distribuição χ2 com ℓ graus de liberdade.


Com modelos ARIMA, testes de portmanteau mais precisos são obtidos se os graus de liberdade forem ajustados para levar em conta o número de parâmetros no modelo. Especificamente, usamos $ℓ−K$ graus de liberdade no teste, onde K é o número de parâmetros AR e MA no modelo. Portanto, para o modelo selecionado, $K=p+q+P+Q=0+1+0+1=2$. O valor de $K$ é passado para a função pelo argumento `dof` como pode-se ver a seguir:


```{r}
augment(fit.agr) |> filter(.model=="autoARIMA") |> features(.innov, ljung_box, lag=8, dof=2)

augment(fit.agr) |> filter(.model=="autoARIMA") |> features(.innov, box_pierce, lag=8, dof=2)
```


Tanto para Ljung-Box quanto Box-Pierce, os resultados não são significativos (ou seja, os p-valores são bem superiores a $0,05$). Assim, podemos concluir que os resíduos não são distinguíveis de uma série estacionária (ruído branco). Isso confirma que o modelo ARIMA selecionado é adequado para a série temporal de produção do setor agropecuário.
...
```{r fig.asp=.5}
fcst.agr <- fit.agr |> select(name, autoARIMA, ARIMA_010.010) |> 
  forecast(h=16)
...
accuracy(fcst.agr, ts1.test) |> select(-.type) |>
  ...
  fmt_number(c(ME,RMSE,MAE,MASE,RMSSE,ACF1), decimals=2) |> 
...
fit |> pivot_longer(!name, names_to = "Modelo", values_to = "Parâmetros")

glance(fit) |> select(name,.model,AIC:BIC) |> arrange(AICc)
```


### 2. Resíduos de previsão

INVESTIGAR OS RESÍDUOS DE PREVISÃO DE SERVIÇOS

```{r}
augment(fit) |> 
  features(.innov, ljung_box, lag=8, dof=c(2,4)) |> 
  rename(lb_pvalue_2dof=lb_pvalue1, lb_pvalue_4dof=lb_pvalue2)

augment(fit) |> 
  features(.innov, box_pierce, lag=8, dof=c(2,4)) |> 
  rename(bp_pvalue_2dof=bp_pvalue1, bp_pvalue_4dof=bp_pvalue2)
```


Tanto para Ljung-Box quanto Box-Pierce, os resultados não são significativos (ou seja, os p-valores são bem superiores a $0,05$). Assim, podemos concluir que os resíduos não são distinguíveis de uma série estacionária (ruído branco). 
...
PORQUE O MAPE ESTÁ ORDENADO ERRADO? O MENOR ERRO VISUALMENTE PARECE SER AGROPECUÁRIA... 
PORQUE NÃO É MELHOR QUE O NAIVE FORECAST?

```{r}
accuracy(fcst, ts1.test) |> select(-.type) |>
  arrange(MAPE) |> 
  gt(rowname_col="stub", locale="pt") |> sub_missing() |>
  fmt_number(c(ME,RMSE,MAE,MASE,RMSSE,ACF1), decimals=2) |> 
  fmt_number(c(MPE,MAPE), decimals=1) |> 
